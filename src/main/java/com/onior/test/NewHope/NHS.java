...
public final class NHS {
    ...
    private void encode(byte[] key, int[] poly) {
        int i, j, b, k, kj, q2;
    ...
    private void decode(int[] poly, byte[] key) {
        int i, j, k;
        int b, t, q2;
        q2 = RLWE_PRIME / 2;
        for (i = 0; i < 32; i++)
    ...
    private void parse(byte[] seed, int[] poly) {
        int i, j;
        int n;
        ...
        for (i = j = 0; i < DEGREE; i++) {
            ...
            n += (int) hash[j + 3] & 0xff;
            j += 4;
            poly[i] = modmul(n, RLWE_ONE);
        }
    }

    private void pack(int[] poly, byte[] array) {
        int i, j, k;
        int a, b, c, d;
    ...
    private void unpack(byte[] array, int[] poly) {
        int i, j, k;
        int a, b, c, d, e, f, g;
    ...
    private void compress(int[] poly, byte[] array) {
        int i, j, k, b;
        int col = 0;
    ...
    private void decompress(byte[] array, int[] poly) {
        int i, j, k, b;
        int col = 0;
    ...
    private void error(RAND RNG, int[] poly) {
        int i, j;
        int n1, n2, r;
        for (i = 0; i < DEGREE; i++) {
    ...
    private void poly_mul(int[] p1, int[] p2, int[] p3) {
        int i;
        for (i = 0; i < DEGREE; i++)
            p1[i] = modmul(p2[i], p3[i]);
    }

    private void poly_add(int[] p1, int[] p2, int[] p3) {
        int i;
        for (i = 0; i < DEGREE; i++)
            p1[i] = (p2[i] + p3[i]);
    }

    private void poly_sub(int[] p1, int[] p2, int[] p3) {
        int i;
        for (i = 0; i < DEGREE; i++)
            p1[i] = (p2[i] + RLWE_PRIME - p3[i]);
    }

    private void poly_soft_reduce(int[] poly) {
        int i;
        int e;
        for (i = 0; i < DEGREE; i++) {
            e = poly[i] - RLWE_PRIME;
    ...
    private void poly_hard_reduce(int[] poly) {
        int i;
        int e;
        for (i = 0; i < DEGREE; i++) {
    ...
    public void SERVER_1(RAND RNG, byte[] SB, byte[] S) {
        int i;
    ...
    public void CLIENT(RAND RNG, byte[] SB, byte[] UC, byte[] KEY) {
        int i;
    ...
    public void SERVER_2(byte[] S, byte[] UC, byte[] KEY) {
        int i;
        SHA3 sh = new SHA3(SHA3.HASH256);
